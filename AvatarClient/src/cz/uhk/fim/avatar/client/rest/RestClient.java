...
public class RestClient {
    ...
    public RestClient(String baseUrl, Resources resources, int resource) {
        this.baseUrl = baseUrl;
        setupHttpClient(resources, resource);
    }
    
    private void setupHttpClient() {
    	setupHttpClient(null, 0);
    }    
    private void setupHttpClient(Resources resources, int resource) {
    	HttpParams httpParams = new BasicHttpParams();
    	HttpConnectionParams.setConnectionTimeout(httpParams, CONNECTION_TIMEOUT);
        HttpConnectionParams.setSoTimeout(httpParams, SOCKET_TIMEOUT);
    	
        if (resources==null) httpClient = new DefaultHttpClient(httpParams);
        else httpClient = new MyHttpClient(httpParams, resources, resource);
    ...
    private Response entityEnclosingRequest(Request r, HttpEntityEnclosingRequestBase request) throws Exception {
    	...
    	if (!r.getPayload().equals("")) {
			request.setEntity(new StringEntity(r.getPayload(), HTTP.UTF_8));
		}
